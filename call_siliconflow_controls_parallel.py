"""
Generate AI-control abstracts with SiliconFlow using concurrent requests.

This is a faster companion to call_siliconflow_controls.py. It keeps the same
CSV shape, skips existing successful rows unless --overwrite is passed, and
writes progress back to the output CSV as requests finish.
...
Environment variables:
    ...
    SILICONFLOW_MODEL     Optional. Defaults to Pro/zai-org/GLM-4.7

Example:
    python .\\call_siliconflow_controls_parallel.py `
      --input .\\real_published_abstracts_ai_control_prompts.csv `
      --out .\\real_published_abstracts_ai_controls_generated.csv `
...
def make_key(row: dict[str, str]) -> tuple[str, str]:
    return row.get("source_id", ""), row.get("prompt_type", "")
...
def chat_completion(
    *,
    ...
    headers = {
        ...
        "User-Agent": "ai-detector-style-study/0.2",
    }
...
def process_one(index: int, row: dict[str, str], args: argparse.Namespace, api_key: str) -> tuple[int, dict[str, str]]:
    ...
    except Exception as exc:  # noqa: BLE001 - row-level failures keep the batch resumable.
...
def main() -> int:
    ...
    parser.add_argument("--workers", type=int, default=10, help="Number of concurrent requests")
    ...
    parser.add_argument("--save-every", type=int, default=1, help="Write CSV after every N completed requests")
    ...
    write_csv_atomic(output_path, output_rows, fieldnames)
    ...
    with ThreadPoolExecutor(max_workers=args.workers) as executor:
        ...
        for future in as_completed(futures):
            ...
            except Exception as exc:  # This should rarely happen because process_one catches row errors.
                result = initialized_row(
                    original,
                    status="error",
                    model=args.model,
            ...
            print(
                ...
                f"{result.get('prompt_type', '')} | {result.get('title', '')[:80]}",
                file=sys.stderr,
            )
